...
const ShortVideoWidget: React.FC<ShortVideoWidgetProps> = ({
  ...
  const {width, height, aspectRatio} = getMediaDimensions(
    videoProps.item.aspectRatio ?? STR_DEFULT_ASPECT_RATIO,
  );
  const thumbnailUrl = getImageUrl(
    videoProps.item.thumbnailUrl,
    width,
    height,
    75,
  );
  ...
  return (
    <View {...rest}>
      <VideoCard
        {...videoProps} // <-- includes geekMode, etc.
