...
export class FeedPrefetchController extends PrefetchController {
  ...
  handleVisibleIndicesChanged(
    ...
    this.logPrefetch(`Visible indices: [${visibleIndices.join(', ')}], prefetch until: ${prefetchUntil}`);

    // Only prefetch forward (don't re-prefetch already processed widgets)
    for (let i = Math.max(this.lastPrefetchedIndex + 1, lastVisible + 1); i <= prefetchUntil; i++) {
    ...
    this.lastPrefetchedIndex = Math.max(this.lastPrefetchedIndex, prefetchUntil);
  }
  ...
  private prefetchWidget(widget: IFeedItem, distance: number): void {
    ...
    if (widget.widgetType === 'carousel') {
      ...
      logger.debug('prefetch', `[FEED] Skipping merch widget at index ${widgetIndex}`);
    }
  }
   ...
   */
  private prefetchCarousel(widget: IFeedItem, widgetIndex: number, basePriority: number): void {
    ...
    const initialVideos: PrefetchVideo[] = videos.slice(0, maxInitial).map((v, idx) => ({
    ...
    if (initialVideos.length > 0) {
      this.logPrefetch(
        ...
        `[${initialVideos.map(v => v.id).join(', ')}] at priority ${basePriority}`
      );
      this.prefetchVideos(initialVideos, basePriority);
    }

    // Low priority: Remaining videos (for potential horizontal scroll)
    const remainingVideos: PrefetchVideo[] = videos.slice(maxInitial).map((v, idx) => ({
  ...
  private prefetchShortVideo(widget: IFeedItem, widgetIndex: number, priority: number): void {
    ...
    this.logPrefetch(`🎯 Short video widget ${widgetIndex}: ${videoId} at priority ${priority}`);
  ...
  onInitialLoad(feedData: IFeedItem[]): void {
    ...
    const range = Math.min(AppConfig.config.visibility.prefetchRange, feedData.length - 1);
  ...
  onPageLoad(allFeedData: IFeedItem[], newPageStartIndex: number): void {
    ...
    this.logPrefetch(`📥 Page loaded: new widgets from index ${newPageStartIndex}`);
