...
const pct = (p: number, arr: number[]) => {
  ...
  const idx = Math.max(
    0,
    Math.min(a.length - 1, Math.floor((p / 100) * (a.length - 1))),
  );
  return a[idx];
};

export const MetricsReportModal = ({
  visible,
  onClose,
}: {
  visible: boolean;
  ...
  const sorted = useMemo(() => {
    ...
    const keyVal = (r: Row) =>
      k === 'startup'
        ? r.startupEffectiveMs ?? Number.POSITIVE_INFINITY
        : k === 'ready'
        ? r.readyMs ?? Number.POSITIVE_INFINITY
        : k === 'stalls'
        ? r.stalls ?? 0
        : r.stallTimeMs ?? 0;
  ...
  const pData = useMemo(() => {
    ...
    return {
      startupP50: pct(50, se),
      startupP90: pct(90, se),
      startupP99: pct(99, se),
      readyP50: pct(50, r),
      readyP90: pct(90, r),
      readyP99: pct(99, r),
      total: rows.length,
  ...
  const exportCSV = async () => {
    ...
    const lines = sorted.map(r =>
      [
        ...
        r.readyMs ?? '',
        r.startupMs ?? '',
        r.startupEffectiveMs ?? '',
        r.readyBeforePlay ? 1 : 0,
        r.stalls ?? 0,
        r.stallTimeMs ?? '',
        r.timeToFirstStallMs ?? '',
        r.eventsCount,
      ].join(','),
    );
    const csv = [header, ...lines].join('\n');
    try {
      await Share.share({message: csv, title: 'metrics.csv'});
    } catch {}
  };
      ...
      <View style={styles.container}>
        <View style={styles.topRow}>
          <Text style={styles.title}>
            Playback Metrics ({pData.total} plays)
          </Text>
        ...
        <View style={styles.controls}>
          ...
          />
          <HeaderButton
            label={descending ? '↓ desc' : '↑ asc'}
            onPress={() => setDescending(d => !d)}
          />
          <HeaderButton
            label={showOnlyStalls ? 'Stalls: ON' : 'Stalls: OFF'}
            onPress={() => setShowOnlyStalls(s => !s)}
          />
