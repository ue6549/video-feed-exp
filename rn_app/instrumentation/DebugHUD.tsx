...
export const DebugHUD: React.FC<HUDProps> = ({
  ...
  const allForPlay = useMemo(
    () => (playId ? metrics.snapshot(playId) : []),
    [version, playId],
  );
    ...
    <View
      ...
      {logs.map((r, i) => (
        <Text key={i} style={{color: '#fff', fontSize: 11}}>
          {r.event}
          {r.dSincePrev != null ? `  +${Math.round(r.dSincePrev)}ms` : ''}
        </Text>
      ))}
    </View>
  );
...
const badge = (label: string, v?: number | string, ms: boolean = true) => (
  <View
    ...
    <Text style={{color: '#fff', fontSize: 11}}>
      {label}: <Text style={{fontWeight: '600'}}>{fmt(v, ms)}</Text>
    </Text>
  </View>
);
