  ...
  while (i < originalArray.length) {
    ...
    if (stepSinceLastMerch > 2 && Math.random() > 0.75) {
      ...
      const feedItem = {
        ...
        id: `short-${i}`,
        data: {
          videoSource: originalArray[i].video_source,
          thumbail: originalArray[i].thumbail,
        },
      };
      newArray.push(feedItem);
      i += 1;
...
const {DEFULT_ASPECT_RATIO, parseAspectRatio, getMediaDimensions, getImageUrl} =
  Utilities;
  ...
  (type, dim, index) => {
    ...
    if (item.widgetType === 'carousel') {
      ...
      const aspectRatioValue =
        parseAspectRatio((item.data as Thumbnail).aspectRatio) ??
        DEFULT_ASPECT_RATIO;
      dim.width = Dimensions.get('window').width;
      dim.height = dim.width / aspectRatioValue;
    } else {
      const aspectRatioValue =
        parseAspectRatio((item.data as VideoData).thumbail.aspectRatio) ??
        DEFULT_ASPECT_RATIO;
      dim.width = Dimensions.get('window').width;
      dim.height = dim.width / aspectRatioValue;
    }
...
function VideoFeed(): JSX.Element {
  ...
  const _renderMerch = (feedItem: IFeedItem) => {
    ...
    const aspectRatioValue =
      parseAspectRatio((feedItem.data as Thumbnail).aspectRatio) ??
      DEFULT_ASPECT_RATIO;
    const {width, height, aspectRatio} = getMediaDimensions(
      (feedItem.data as Thumbnail).aspectRatio,
    );
    const imageUrl = getImageUrl(
      (feedItem.data as Thumbnail).dynamicImageUrl,
      width,
      height,
      75,
    );
  ...
  const rowRenderer = (type: string | number, item: IFeedItem) => {
    if (item.widgetType === 'carousel') {
      ...
      const {width, height, aspectRatio} = getMediaDimensions(
        (item.data as VideoData).thumbail.aspectRatio,
      );
      const thumbnailUrl = getImageUrl(
        (item.data as VideoData).thumbail.dynamicImageUrl,
        width,
        height,
        75,
      );
