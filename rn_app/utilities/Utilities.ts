...
export const getMediaDimensions = (
  ...
  if (width && !height) {
    return {
      ...
      height: Math.ceil(width / aspectRatioValue),
      aspectRatio: aspectRatioValue,
    };
  } else if (!width && height) {
    return {
      width: Math.ceil(height * aspectRatioValue),
    ...
    return {
      ...
      height: Math.ceil(FALLBACK_MEDIA_WIDTH / aspectRatioValue),
      aspectRatio: aspectRatioValue,
    }; // Default size
  }
};
...
export const getImageUrl = (
  ...
  return parameterizedUrl
    .replace('{@width}', `${width ? width : 1000}`)
