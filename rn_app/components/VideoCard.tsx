...
} from './native_components/VisibilityTrackingView';
...
const toRawVisibilityConfig = (
  config: VisibilityTransitioningConfig,
    ...
    movingIn: [
      ...
      config.movingIn.isActive,
    ],
    movingOut: [
      config.movingOut.willResignActive,
      config.movingOut.notActive,
      config.movingOut.released,
    ],
  };
};
...
const VideoCard: React.FC<VideoCardProps> = ({
  ...
  const now = () => Date.now();
    ...
    useEffect(() => {
        ...
        logger.info(
          ...
          `[${
            item.id
          }] ✅ Set isPlayerPlaying=false, paused prop will be: ${true}`,
        );
        // Show play button for low-end devices or manual play
        if (
          AppConfig.config.performance.isLowEndDevice ||
          !AppConfig.config.performance.autoplayOnLowEnd
        ) {
  ...
  const onVisibilityChange = (event: {
    ...
    logger.debug(
      'visibility',
      `[${item.id}] ${direction} ${visibilityPercentage}%`,
    );
    ...
    let newState: MediaCardVisibility | null = null;
      ...
      if (visibilityPercentage <= THRESHOLDS.movingOut.released) {
        ...
        visibilityPercentage <= THRESHOLDS.movingOut.willResignActive
      ) {
    ...
    if (newState && newState !== lastVisibilityStateRef.current) {
      ...
      if (shouldHavePlayer && !isPlayerAttached) {
        ...
        logger.info(
          'video',
          `[${item.id}] 🔌 Detaching player BEFORE state emission`,
        );
  ...
  const onReadyForDisplay = () => {
    ...
    logger.info(
      'video',
      `[${item.id}] ✅ Video ready for display (load time: ${loadTime}ms)`,
    );
